...
function generateHeader(doc) {
  doc
    ...
    .text("Messala Street", 200, 65, { align: "right" })
    .text("Fayoum,Egypt", 200, 80, { align: "right" })
    .moveDown();
}
...
function generateCustomerInformation(doc, invoice) {
  ...
  doc
    .fontSize(10)
    .text("User ID :", 50, customerInformationTop)
    .font("Helvetica-Bold")
    ...
    .text(
      formatCurrency(
        invoice.subtotalPrice * (1 - (invoice.couponDiscount || 0) / 100)
      ) + " (E.G.P)",
      150,
...
function generateInvoiceTable(doc, invoice) {
  ...
  generateTableRow(
    ...
    "coupon Discount",
    "",
    formatPercentage(invoice.couponDiscount || 0)
  );
...
function formatPercentage(cents) {
  return "%" + cents.toFixed(2);
}
